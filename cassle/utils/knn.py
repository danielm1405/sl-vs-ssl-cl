...
class WeightedKNNClassifier(Metric):
    ...
    def compute(self) -> Dict[str, float]:
        ...
        # task agnostic
        knn1, knn5 = self.compute_one(train_features, train_targets, test_features, test_targets)
        results_dict.update({"val_knn_acc1": knn1, "val_knn_acc5": knn5})

        # task aware
        if self.task_aware:
            for task_idx, task in enumerate(self.tasks):
                train_task_mask = torch.isin(train_targets, torch.tensor(task, device=device))
                test_task_mask = torch.isin(test_targets, torch.tensor(task, device=device))

                train_task_targets = train_targets[train_task_mask]
                test_task_targets = test_targets[test_task_mask]

                # remapping
                for idx, c in enumerate(sorted(list(task))):
                    train_task_targets[train_task_targets == c] = idx
                    test_task_targets[test_task_targets == c] = idx

                knn1, knn5 = self.compute_one(train_features[train_task_mask], train_task_targets,
                                              test_features[test_task_mask], test_task_targets)
                results_dict[f"val_knn_acc1_task{task_idx}"] = knn1
                results_dict[f"val_knn_acc5_task{task_idx}"] = knn5
